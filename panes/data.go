...
func MyJson(action string) {
	...
	if action == "LOAD" {
		...
		Queuepassword = MyApp.Preferences().StringWithFallback("Queuepassword", xQueuepassword)
		xPasswordMinimumSize, _ := Encrypt("6", MySecret)
		PasswordMinimumSize = MyApp.Preferences().StringWithFallback("PasswordMinimumSize", xPasswordMinimumSize)
...
func MyHash(action string, hash string) bool {
